...
ARG DEVSTACK_BRANCH="stable/queens"
ARG PROJECTS_BRANCH="stable/queens"
...
RUN git clone https://git.openstack.org/openstack-dev/devstack --branch $DEVSTACK_BRANCH && \
    ...
    do \
        git clone \
            https://git.openstack.org/openstack/$PROJECT.git \
            /opt/stack/$PROJECT \
...
#RUN git clone https://git.openstack.org/openstack/kuryr-libnetwork.git /opt/stack/kuryr-libnetwork  --branch stable/queens --depth 1 --single-branch;
...
RUN curl --silent --show-error https://raw.githubusercontent.com/pypa/get-pip/main/public/2.7/get-pip.py | python
...
COPY upper-constraints.txt /opt/stack/requirements/
COPY global-requirements.txt /opt/stack/requirements/
COPY test-requirements.txt /opt/stack/requirements/
...
LABEL org.label-schema.build-date=$BUILD_DATE \
        ...
        org.label-schema.docker.cmd="docker run --cap-add=NET_ADMIN --cap-add  Add Linux capabilities --privileged --detach devstack " \
